...
ml_list_custom_models <- function(
    x,
    concurrency=1,
    concurrentTimeout=NULL,
    errorCol="ListCustomModels_c4bc82baac3b_error",
    handler=NULL,
    op=NULL,
    opCol=NULL,
    outputCol="ListCustomModels_c4bc82baac3b_output",
    subscriptionKey=NULL,
    subscriptionKeyCol=NULL,
    timeout=60.0,
    url=NULL,
    only.model=FALSE,
    uid=random_string("ml_list_custom_models"),
    ...)
{
    if (unfit.model) {
        sc <- x
    ...
    scala_class <- "com.microsoft.azure.synapse.ml.cognitive.ListCustomModels"
