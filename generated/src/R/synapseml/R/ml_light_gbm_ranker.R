...
#' @param evalAt NDCG and MAP evaluation positions, separated by comma
...
#' @param labelGain graded relevance for each label in NDCG
...
#' @param maxPosition optimized NDCG at this position
...
ml_light_gbm_ranker <- function(
    ...
    unfit.model=FALSE,
    only.model=FALSE,
    uid=random_string("ml_light_gbm_ranker"),
    ...)
{
    if (unfit.model) {
        sc <- x
    ...
    scala_class <- "com.microsoft.azure.synapse.ml.lightgbm.LightGBMRanker"
    ...
    mod_parameterized <- mod %>%
        ...
        invoke("setEvalAt", as.array(evalAt)) %>%
        ...
        invoke("setLabelGain", as.array(labelGain)) %>%
        ...
        invoke("setMaxPosition", as.integer(maxPosition)) %>%
    ...
    transformer <- mod_parameterized %>%
        invoke("fit", df)
    scala_transformer_class <- "com.microsoft.azure.synapse.ml.lightgbm.LightGBMRankerModel"
    if (only.model)
