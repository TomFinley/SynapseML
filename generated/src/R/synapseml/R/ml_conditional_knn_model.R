...
#' ConditionalKNNModel
#'
#' @param ballTree the ballTree model used for perfoming queries
...
ml_conditional_knn_model <- function(
    x,
    ballTree=NULL,
    conditionerCol=NULL,
    featuresCol=NULL,
    ...
    valuesCol=NULL,
    only.model=FALSE,
    uid=random_string("ml_conditional_knn_model"),
    ...)
{
    if (unfit.model) {
        sc <- x
    ...
    scala_class <- "com.microsoft.azure.synapse.ml.nn.ConditionalKNNModel"
