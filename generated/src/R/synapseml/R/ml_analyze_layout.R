...
#' @param language The BCP-47 language code of the text in the document. Layout supports auto language identification and multilanguage documents, so only provide a language code if you would like to force the documented to be processed as that specific language.
...
ml_analyze_layout <- function(
    x,
    backoffs=c(100,500,1000),
    concurrency=1,
    concurrentTimeout=NULL,
    errorCol="AnalyzeLayout_a1706a4066c6_error",
    imageBytes=NULL,
    imageBytesCol=NULL,
    imageUrl=NULL,
    imageUrlCol=NULL,
    ...
    outputCol="AnalyzeLayout_a1706a4066c6_output",
    pages=NULL,
    pagesCol=NULL,
    pollingDelay=300,
    readingOrder=NULL,
    readingOrderCol=NULL,
    ...
    url=NULL,
    only.model=FALSE,
    uid=random_string("ml_analyze_layout"),
    ...)
{
    if (unfit.model) {
        sc <- x
    ...
    scala_class <- "com.microsoft.azure.synapse.ml.cognitive.AnalyzeLayout"
    ...
    mod_parameterized <- mod %>%
        ...
        invoke("setReadingOrderCol", readingOrderCol) %>%
        invoke("setReadingOrder", readingOrder) %>%
