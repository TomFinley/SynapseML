...
#' @param tweedieVariancePower control the variance of tweedie distribution, must be between 1 and 2
...
ml_light_gbm_regressor <- function(
    x,
    alpha=0.9,
    baggingFraction=1.0,
    baggingFreq=0,
    ...
    unfit.model=FALSE,
    only.model=FALSE,
    uid=random_string("ml_light_gbm_regressor"),
    ...)
{
    if (unfit.model) {
        sc <- x
    ...
    scala_class <- "com.microsoft.azure.synapse.ml.lightgbm.LightGBMRegressor"
    ...
    mod_parameterized <- mod %>%
        ...
        invoke("setTweedieVariancePower", as.double(tweedieVariancePower)) %>%
    ...
    transformer <- mod_parameterized %>%
        invoke("fit", df)
    scala_transformer_class <- "com.microsoft.azure.synapse.ml.lightgbm.LightGBMRegressionModel"
    if (only.model)
