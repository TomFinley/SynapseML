...
#' @param includeOpinionMining includeOpinionMining option
...
ml_text_sentiment_sdk <- function(
    x,
    batchSize=5,
    concurrency=1,
    ...
    errorCol="Error",
    includeOpinionMining=NULL,
    includeOpinionMiningCol=NULL,
    ...
    url=NULL,
    only.model=FALSE,
    uid=random_string("ml_text_sentiment_sdk"),
    ...)
{
    if (unfit.model) {
        sc <- x
    ...
    scala_class <- "com.microsoft.azure.synapse.ml.cognitive.TextSentimentSDK"
    ...
    mod_parameterized <- mod %>%
        ...
        invoke("setIncludeOpinionMiningCol", includeOpinionMiningCol) %>%
        invoke("setIncludeOpinionMining", includeOpinionMining) %>%
