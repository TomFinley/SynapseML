...
ml_isolation_forest <- function(
    ...
    unfit.model=FALSE,
    only.model=FALSE,
    uid=random_string("ml_isolation_forest"),
    ...)
{
    if (unfit.model) {
        sc <- x
    ...
    scala_class <- "com.microsoft.azure.synapse.ml.isolationforest.IsolationForest"
    ...
    transformer <- mod_parameterized %>%
        invoke("fit", df)
    scala_transformer_class <- "com.microsoft.azure.synapse.ml.isolationforest.IsolationForestModel"
    if (only.model)
