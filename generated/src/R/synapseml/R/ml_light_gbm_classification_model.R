...
#'
#' @param actualNumClasses Inferred number of classes based on dataset metadata or, if there is no metadata, unique count
...
ml_light_gbm_classification_model <- function(
    x,
    actualNumClasses=NULL,
    ...
    thresholds=NULL,
    only.model=FALSE,
    uid=random_string("ml_light_gbm_classification_model"),
    ...)
{
    if (unfit.model) {
        sc <- x
    ...
    scala_class <- "com.microsoft.azure.synapse.ml.lightgbm.LightGBMClassificationModel"
    mod <- invoke_new(sc, scala_class, uid = uid)
    mod_parameterized <- mod %>%
        invoke("setActualNumClasses", as.integer(actualNumClasses)) %>%
