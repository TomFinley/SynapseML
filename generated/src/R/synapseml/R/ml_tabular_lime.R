...
ml_tabular_lime <- function(
    ...
    unfit.model=FALSE,
    only.model=FALSE,
    uid=random_string("ml_tabular_lime"),
    ...)
{
    if (unfit.model) {
        sc <- x
    ...
    scala_class <- "com.microsoft.azure.synapse.ml.lime.TabularLIME"
    ...
    transformer <- mod_parameterized %>%
        invoke("fit", df)
    scala_transformer_class <- "com.microsoft.azure.synapse.ml.lime.TabularLIMEModel"
    if (only.model)
