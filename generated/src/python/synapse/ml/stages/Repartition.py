...
class Repartition(ComplexParamsMixin, JavaMLReadable, JavaMLWritable, JavaTransformer):
    ...
    disable = Param(Params._dummy(), "disable", "Whether to disable repartitioning (so that one can turn it off for evaluation)", typeConverter=TypeConverters.toBoolean)
    
    n = Param(Params._dummy(), "n", "Number of partitions", typeConverter=TypeConverters.toInt)
    ...
    def __init__(
        ...
        if java_obj is None:
            self._java_obj = self._new_java_obj("com.microsoft.azure.synapse.ml.stages.Repartition", self.uid)
        else:
    ...
    def getJavaPackage():
        ...
        return "com.microsoft.azure.synapse.ml.stages.Repartition"
